...
export const SiteBanner = mongoose.models.BravoSiteBanner || mongoose.model('BravoSiteBanner', new mongoose.Schema({ _id: String, alerts: [String], revision: { type: Number, default: 0 }, updatedBy: mongoose.Schema.Types.ObjectId }, { timestamps: true }));
const input = z.object({ expectedRevision: z.number().int().min(0), alerts: z.array(z.string().trim().min(1).max(280)).max(30) }).strict();
const connect = async (_req, _res, next) => { await connectDb(); next(); };
const publicBanner = value => ({ alerts: value?.alerts || [], revision: value?.revision || 0 });
