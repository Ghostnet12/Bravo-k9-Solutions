...
try {for(const [engineName,engine]of Object.entries({chromium,webkit})){const browser=await engine.launch();try{for(const width of [390,1440]){
 ...
 try{
  ...
  await page.getByRole('button',{name:'New lesson',exact:true}).click();await page.getByLabel('Lesson ID',{exact:true}).fill('body-language');await page.getByLabel('Title',{exact:true}).fill('Read your dog');await page.getByLabel('Instructor',{exact:true}).selectOption('David and Ashley');await page.getByLabel('Section',{exact:true}).selectOption('section-one');await page.getByLabel('Lesson format',{exact:true}).selectOption('text');await page.getByLabel('Description',{exact:true}).fill('Learn the signals.');await page.getByLabel('Lesson instructions',{exact:true}).fill('Look for relaxed posture. Give your dog space.');await page.getByRole('button',{name:'Save draft',exact:true}).click();await page.getByText('Draft saved. You can now upload its private media.',{exact:true}).waitFor();
 ...
 }catch(error){await page.screenshot({path:`test-results/lesson-failure-${engineName}-${width}.png`,fullPage:true});await writeFile(`test-results/lesson-failure-${engineName}-${width}.json`,JSON.stringify({error:error.message,errors,text:await page.locator('body').innerText()},null,2));throw error;}finally{await context.close();}
