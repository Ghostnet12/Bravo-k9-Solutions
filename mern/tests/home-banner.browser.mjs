  ...
  for (const [engineName, engine] of Object.entries({ chromium, webkit })) {
    ...
    try {
      for (const width of [390, 1440]) {
        ...
        await page.route('**/api/**', async route => {
          ...
          if (path === '/api/site-banner') {
            if (route.request().method() === 'PUT') {
              ...
              else { const body = route.request().postDataJSON(); assert.equal(body.expectedRevision, saved.revision); saved = { alerts: body.alerts, revision: saved.revision + 1 }; json = saved; }
            } else json = saved;
          }
        ...
        try {
          await page.goto(origin); await page.getByRole('button', { name: 'Edit alerts', exact: true }).waitFor();
          ...
          failSave = true; await page.getByRole('button', { name: 'Publish alerts', exact: true }).click(); await page.getByRole('alert').waitFor();
          assert.ok((await page.getByLabel('Alert 2', { exact: true }).inputValue()).includes('New training notice'));
          failSave = false; await page.getByRole('button', { name: 'Publish alerts', exact: true }).click(); await page.getByRole('dialog').waitFor({ state: 'hidden' });
          assert.ok((await banner.innerText()).includes('New training notice'));
          await page.reload(); await page.getByRole('button', { name: 'Edit alerts', exact: true }).waitFor();
          assert.ok((await banner.innerText()).includes('New training notice'));
          ...
          await page.getByRole('dialog').waitFor(); await page.getByRole('button', { name: 'Close alert editor' }).click();
