...
try {for(const [engineName,engine]of Object.entries({chromium,webkit})){const browser=await engine.launch();try{for(const width of [390,1440]){
 const context=await browser.newContext({viewport:{width,height:900},hasTouch:true}),page=await context.newPage(),errors=[];let signedIn=false, trainingDisabled=false;const writes=[];
 await page.addInitScript(()=>{window.__renderErrors=[];window.addEventListener('error',event=>window.__renderErrors.push({message:event.message,stack:event.error?.stack,cause:event.error?.cause?.message,causeStack:event.error?.cause?.stack,url:location.href}));window.addEventListener('error',event=>console.error('Render cause:',event.error?.cause?.stack||event.error?.stack));});
 ...
 try{
  ...
  await page.screenshot({path:`test-results/first-visit-${engineName}-${width}.png`});await page.getByRole('button',{name:'Choose a first visit →',exact:true}).click();await page.getByRole('heading',{name:'Choose your first visit',exact:true}).waitFor();
  assert.equal(await page.locator('.program-choices').isVisible(),false);assert.equal(await page.getByRole('button',{name:'Find available days & times',exact:true}).isVisible(),false);assert.equal(await page.locator('.single-date-editor').isVisible(),false);assert.equal(await page.locator('.first-visit-date-range').getAttribute('open'),null);await page.getByText('Choose your visit time',{exact:true}).waitFor();
  ...
  await page.getByRole('heading',{name:'Choose your first visit',exact:true}).waitFor();await page.getByLabel('Choose my trainer',{exact:true}).selectOption('111111111111111111111111');
  assert.equal(await page.getByLabel('Dogs’ names',{exact:true}).inputValue(),'Biscuit');assert.equal(await page.getByLabel('Anything the team should know?',{exact:true}).inputValue(),'Jumps when visitors arrive.');assert.equal(await page.getByLabel('Training focus',{exact:true}).inputValue(),'puppy-foundations');
  assert.equal(await page.getByText('IMPORTANT APPOINTMENT NOTICE',{exact:true}).count(),1);await dates.first().click();await page.locator('.bulk-times').getByRole('button',{name:/10:00/}).click();assert.equal(await page.locator('.bulk-times button[aria-pressed=true]').count(),1);await page.screenshot({path:`test-results/first-visit-schedule-${engineName}-${width}.png`,fullPage:true});await dates.first().click();
  await page.getByRole('button',{name:'Plan multiple visits instead',exact:true}).click();await page.getByRole('heading',{name:'Build your schedule',exact:true}).waitFor();assert.equal(await page.locator('.program-choices').isVisible(),true);await page.getByRole('button',{name:'Find available days & times',exact:true}).click();await dates.first().click();await dates.nth(1).click();assert.equal(await page.locator('.date-grid button[aria-pressed=true]').count(),2);
  ...
  assert.deepEqual(writes,['/api/auth/login']);if(errors.length)console.log('Render diagnostics',consoleErrors,await page.evaluate(()=>window.__renderErrors));assert.deepEqual(errors,[]);console.log(`PASS ${engineName}/${width}: short first step, accurate multi-dog price, sign-in resume, preserved notes, full scheduler and quiet banner`);
