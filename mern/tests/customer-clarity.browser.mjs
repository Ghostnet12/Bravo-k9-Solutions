...
try {for(const [engineName,engine]of Object.entries({chromium,webkit})){const browser=await engine.launch();try{for(const width of [390,1440]){
 ...
 try{
  ...
  await page.screenshot({path:`test-results/first-visit-${engineName}-${width}.png`});await page.getByRole('button',{name:'Choose a first visit →',exact:true}).click();await page.getByRole('heading',{name:'Sign in to choose your trainer.',exact:true}).waitFor();
  ...
  await page.getByRole('link',{name:'Sign in or create an account',exact:true}).click();await page.getByLabel('Name or email',{exact:true}).fill('Fixture Client');await page.getByLabel('Password',{exact:true}).fill('isolated-fixture-password');await page.locator('form').getByRole('button',{name:'Sign in',exact:true}).click();
