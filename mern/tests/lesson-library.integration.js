...
test('lesson studio, closed-library boundaries and paid access', {timeout:180000}, async t=>{
 ...
 try {
  ...
  await t.test('default closed: no public catalog, paid media or purchase; owner and administrator can prepare',async()=>{
   ...
   const config=(await call(null,'get','/api/config').expect(200)).body;assert.equal(config.lessonLibrary.open,false);assert.equal(config.lessonLibrary.pendingCheckouts,undefined,'operational checkout counts stay private');assert.equal((await call('owner','get','/api/admin/lesson-library')).body.library.pendingCheckouts,1,'initial closed mode reconciles pre-deployment checkout links');assert.ok((await LessonLibrary.findById('library')).cleanupAfter);
   let expired=false;assert.equal(await expireLessonCheckouts({checkout:{sessions:{retrieve:async()=>({id:'cs_before_deployment',status:'open'}),expire:async()=>{expired=true;}}}}),0);assert.equal(expired,true);await Booking.deleteOne({_id:oldCheckout._id});assert.equal(config.services.find(s=>s.id==='online').cents,7500);assert.equal(config.services.find(s=>s.id==='online').enabled,false);
