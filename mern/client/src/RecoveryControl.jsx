...
export default function RecoveryControl({ person }) {
  ...
  if (person.role !== 'member' || person.blocked) return null;
  if (!person.email) return <p className="helper">You can manage this member’s schedule without contact details. To enable client sign-in later, add their email and save the profile, then use Account recovery.</p>;
