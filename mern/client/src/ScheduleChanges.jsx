...
export default function ScheduleChanges({data,initialMonth,onSaved,onClose}) {
  ...
  return <section className="schedule-change-editor" aria-label="Add Days and Times"><button type="button" className="button button-ghost editor-back" disabled={busy} onClick={onClose}>{dirty ? 'Discard changes & back' : 'Back to calendar'}</button><h3>Add Days and Times</h3><ol className="schedule-help" aria-label="Scheduling steps"><li>Select your dates</li><li>Choose a time for those dates</li><li>Review and save</li></ol><p className="helper">For a different time on one day, open “Change one date” below.</p>
    {!bookings.length?<p>No paid training request is available to edit. Contact Bravo for help.</p>:<form onSubmit={save}>
      ...
      <p className="helper">Changes within 24 hours follow Bravo’s appointment policy. Cancelling visits does not automatically extend membership. {isStaff ? 'To make up a missed training day, return to the calendar and use Credit days.' : 'Bravo can credit days for missed training.'}</p>
      {!dirty && !pendingDates.length && <p className="helper">No changes to save yet. Select dates and choose a time, or cancel selected visits.</p>}<Notice error>{error}</Notice><div className="record-actions"><button className="button" disabled={busy||loading||!dirty||pendingDates.length>0}>{busy?'Saving…':'Save changes'}</button><button type="button" className="quiet-button" disabled={busy} onClick={()=>{setDraft({});setBatchDates([]);setDay('');setNote('');setError('')}}>Reset changes</button></div>
    </form>}
