...
import { creditedCalendarDates, creditedEnd, lastCoveredDay, termForDayCredit } from '../../shared/day-credits';
...
export default function ScheduleChanges({data,initialMonth,onSaved,onClose}) {
  ...
  const [batchDates,setBatchDates]=useState([]),[giveCredit,setGiveCredit]=useState(false);
  ...
  const missedDate=batchDates.length===1&&savedDates.has(batchDates[0])?batchDates[0]:'';
  const creditTerm=isStaff?termForDayCredit(data.terms,booking,missedDate):null;
  const alreadyCredited=!!missedDate&&(data.dayCredits||[]).some(credit=>credit.missedDate===missedDate);
  const canCredit=!!creditTerm&&!alreadyCredited&&!dirty;
  const newCreditDate=creditTerm?lastCoveredDay(creditedEnd(creditTerm.validUntil,1)):'';
  ...
  function toggleDay(date) {
    setError('');setGiveCredit(false);setBatchDates(dates=>dates.includes(date)?dates.filter(d=>d!==date):[...dates,date].sort());
  }
  ...
  async function save(e) {
    ...
    if(creditMode){
      const body={clientId:data.client.id,termId:creditTerm.stripeId,expectedEnd:creditTerm.validUntil,days:1,missedDate,note};
      ...
      try{
        ...
        const focusDate=lastCoveredDay(r.credit.afterEnd);
        onSaved(`Saved. Training on ${formatDate(missedDate)} is cancelled. ${formatDate(focusDate)} is marked Credit at the end of the membership.`,focusDate);
  ...
  return <section className="schedule-change-editor" aria-label="Add or Cancel Date"><button type="button" className="button button-ghost editor-back" disabled={busy} onClick={onClose}>{hasChanges ? 'Discard changes & back' : 'Back to calendar'}</button><h3>Add or Cancel Date</h3><ol className="schedule-help" aria-label="Scheduling steps"><li>Select your dates</li><li>{isStaff?'Choose a time, cancel, or give credit':'Choose a time or cancel'}</li><li>Review and save</li></ol><p className="helper">{isStaff?'To credit a missed day, select its date, choose “Give credit for this day”, then save.':'For a different time on one day, open “Change one date” below.'}</p>
    {!bookings.length?<p>No paid training request is available to edit. Contact Bravo for help.</p>:<form onSubmit={save}>
      ...
      {showTimes&&canOpen&&<label className="check-label"><input type="checkbox" checked={openWeekends} disabled={busy} onChange={e=>setOpenWeekends(e.target.checked)}/>Open the selected weekend times when saving</label>}
      ...
      <ScheduleDayGrid month={month} onMonth={setMonth} disabled={busy||loading} onDay={toggleDay} dayState={date=>{
        ...
        return {selected:batchDates.includes(date),changed,credited:credited.has(date),disabled:!booking||(!times.length&&!base.length&&!free.length&&!(isStaff&&savedDates.has(date))),marker:creditMode&&date===missedDate?'×':changed?(times.length?'+':'×'):(times.length||savedCounts[date]||(savedDates.has(date)?'×':'')),label:`${creditMode&&date===missedDate?'Cancel and credit, ':''}${savedDates.has(date)?`${savedCounts[date]||0} scheduled training visits, `:''}${times.length} saved or drafted visit${times.length===1?'':'s'}${changed?(times.length?', pending changes':', pending cancellation'):''}${credited.has(date)?', credited membership day':''}`};
      }}/>
      {isStaff&&<div className="calendar-credit-action">
        <label className="check-label"><input type="checkbox" checked={creditMode} disabled={busy||!canCredit} onChange={e=>{setGiveCredit(e.target.checked);setError('')}}/>Give credit for this day</label>
        {creditMode?<div className="credit-preview" role="status"><p>Cancel training on <strong>{formatDate(missedDate)}</strong>.</p><p>Add <strong>{formatDate(newCreditDate)} · Credit</strong> at the end of the membership.</p></div>:<p className="helper">{dirty?'Save or reset your time changes before giving a day credit.':alreadyCredited?'This day has already been credited.':!missedDate?'Select one scheduled training date to cancel it and add one credit day.':!creditTerm?'This date needs one linked training membership with manual renewal. Ask the owner to check its membership dates.':'Cancel this day and add one day to the end of the membership. Your note is optional.'}</p>}
      </div>}
      ...
      {!creditMode&&<p className="helper">Changes within 24 hours follow Bravo’s appointment policy. Cancelling visits does not automatically extend membership. {isStaff ? 'Select one scheduled date and choose “Give credit for this day” to extend the membership.' : 'Bravo can credit days for missed training.'}</p>}
