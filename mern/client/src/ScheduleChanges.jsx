...
export default function ScheduleChanges({data,initialMonth,onSaved,onClose}) {
  ...
  function toggleDay(date) {
    ...
    if(selected(date).length) setDraft(d=>({...d,[date]:[]}));
      ...
      const free=options(date).filter(time=>available(date,time)),usual=booking?.visits.find(v=>v.service==='training')?.time;
      const restored=original[date]?.length?original[date]:[free.includes(usual)?usual:free[0]].filter(Boolean);
  ...
  return <section className="schedule-change-editor" aria-label="Add Days and Times"><h3>Add Days and Times</h3><p>Tap a day to add it. Tap again to take it off. Choose times below the calendar, then save all changes together.</p>
    {!bookings.length?<p>No paid training request is available to edit. Contact Bravo for help.</p>:<form onSubmit={save}>
      ...
      <ScheduleDayGrid month={month} onMonth={setMonth} disabled={busy||loading} onDay={toggleDay} dayState={date=>{
        ...
        return {selected:times.length>0,changed,disabled:!booking||(!times.length&&!base.length&&!free.length),marker:changed?(times.length?'+':'×'):(times.length||''),label:`${times.length} selected visit${times.length===1?'':'s'}${changed?(times.length?', pending changes':', pending cancellation'):''}`};
      }}/>
      {day&&<fieldset disabled={busy} className="calendar-time-options"><legend>Times for {formatDate(day)}</legend>{!selected(day).length&&<p>This day is off in your draft. Tap it again to restore it, or choose a time below.</p>}<div className="check-grid">{options(day).map(time=><label className="check-label" key={time}><input type="checkbox" disabled={!selected(day).includes(time)&&!available(day,time)} checked={selected(day).includes(time)} onChange={()=>setDraft(d=>({...d,[day]:selected(day).includes(time)?selected(day).filter(t=>t!==time):[...selected(day),time].sort()}))}/>{formatTime(time)}{!available(day,time)&&!selected(day).includes(time)&&(hours[day]?.reservedTimes?.includes(time)?' · Reserved':' · Outside trainer hours')}</label>)}</div></fieldset>}
