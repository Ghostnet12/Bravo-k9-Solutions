...
export default function FirstVisitIntro({ form, setForm, dogCount, setDogCount, focus, setFocus, cents, onContinue, onFullSchedule }) {
  return <Editable as="section" contentKey="first-visit-intro" className="panel first-visit-intro">
    ...
    <div className="price-total"><span>Private training · {dogCount} {dogCount === 1 ? 'dog' : 'dogs'}</span><strong>{money(cents + (dogCount - 1) * TRAINING_ADDITIONAL_DOG_CENTS)}/month</strong></div>
    ...
    <form onSubmit={event => { event.preventDefault(); onContinue(); }}>
      <div className="form-grid"><label>Dog’s name<input required maxLength={80} autoComplete="off" value={form.dogName} onChange={event => setForm(old => ({ ...old, dogName:event.target.value }))}/></label>
      <label>Number of dogs<input type="number" min="1" max="10" required value={dogCount} onChange={event => setDogCount(Math.max(1,Math.min(10,Number(event.target.value)||1)))}/></label></div>
      <label>What would you like help with?<select value={focus} onChange={event => setFocus(event.target.value)}>{TRAINING_FOCUSES.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label>Anything you’d like your trainer to know? (optional)<textarea rows="3" maxLength={1500} value={form.notes} onChange={event => setForm(old=>({...old,notes:event.target.value}))} placeholder="For example: pulling on walks or jumping on visitors"/></label>
