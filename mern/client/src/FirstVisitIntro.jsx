...
export default function FirstVisitIntro({ form, setForm, dogCount, setDogCount, focus, setFocus, cents, withLessons = false, onContinue, onFullSchedule }) {
  return <Editable as="section" contentKey="first-visit-intro" className="panel first-visit-intro">
    ...
    <div className="price-total"><span>{withLessons ? 'Training + lessons' : 'Private training'} · {dogCount} {dogCount === 1 ? 'dog' : 'dogs'}</span><strong>{money(cents + (dogCount - 1) * TRAINING_ADDITIONAL_DOG_CENTS)}<small>/month</small></strong></div>
    ...
    {withLessons && <p>Includes online lesson access for one month. Save $25 compared with purchasing training and lessons separately.</p>}
