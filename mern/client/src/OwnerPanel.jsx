...
export default function OwnerPanel({ user }) {
  ...
  async function addClient(event) {
    ...
    try {
      ...
      setCreated(result); form.reset(); setClientStartDate(''); setQuery(result.user.email); setUsers([{ ...result.user, _id: result.user.id, membership: { revision: 0, enabled: false, manual: false, paidMembership: !!result.membership?.active } }]);
  ...
  return <section id="owner-controls"><div className="section-label"><div><p className="kicker gold">ACCESS MANAGEMENT</p><h2>People & permissions.</h2></div></div><p>Find an existing customer by name or email, or use assisted onboarding to create a client account for someone who needs help. Member access is separate from Staff and Administrator work permissions.</p><Notice error>{error}</Notice><Notice>{notice}</Notice>
    <MembershipSetup/><details className="panel add-client-panel" key={`add-client-${resetVersion}`}><summary>Add a client</summary><p>Create a client-only account. For an existing customer, enter their training month’s actual start date below. No new charge is made.</p><form onSubmit={addClient}><div className="form-grid"><label>Client name<input name="name" autoComplete="name" minLength="2" maxLength="80" required/></label><label>Email<input name="email" type="email" autoComplete="email" maxLength="254" required/></label><label>Phone<input name="phone" type="tel" autoComplete="tel" maxLength="30"/></label><label>Dog’s name<input name="dogName" autoComplete="off" maxLength="80"/></label><label className="full-width">Visit address<input name="address" autoComplete="street-address" maxLength="300"/></label></div><MembershipDateFields value={clientStartDate} onChange={setClientStartDate} disabled={creating} optional name="membershipStartDate" label="Training membership start date"/>{clientStartDate && <label>Dogs covered by this training month<input name="trainingDogCount" type="number" min="1" max="10" defaultValue="1" required disabled={creating}/></label>}<button className="button button-small" disabled={creating}>{creating ? 'Creating client…' : 'Create client account'}</button></form>{created && <div className="created-client notice" role="status"><h3>Client account created.</h3>{created.membership && <p>Training membership dates saved. {created.membership.active ? 'The client currently has training access.' : 'Access follows the selected dates; a past month is not restarted today.'} <Link className="inline-link" to={`/schedule?client=${created.user.id}`}>Open the client’s schedule</Link></p>}<p><strong>{created.user.name}</strong> can sign in with {created.user.email} and the temporary password below.</p><label>Temporary password<input value={created.temporaryPassword} readOnly onFocus={event => event.currentTarget.select()}/></label><p className="helper"><strong>This password is shown only here.</strong> Share it privately and ask the client to change it from their Account page after signing in.</p><button className="quiet-button" type="button" onClick={() => setCreated(null)}>Hide temporary password</button></div>}</details>
