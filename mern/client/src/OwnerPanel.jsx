...
export default function OwnerPanel({ user }) {
  ...
  const [membershipFormVersion, setMembershipFormVersion] = useState(0);
  ...
  async function addClient(event) {
    ...
    try {
      ...
      const { manualMembershipEnabled, membershipStartDate, membershipServices, membershipDogCount, ...account } = fields;
      const result = await api('/admin/users', { method: 'POST', body: { ...account, ...(manualMembershipEnabled ? { membership: membershipFromForm(fields) } : {}) } });
      setCreated(result); form.reset(); setMembershipFormVersion(value => value + 1); setQuery(result.user.email); setUsers([{ ...result.user, _id: result.user.id, membership: result.membership || { revision: 0, manual: false, enabled: false } }]);
