...
export default function OwnerPanel({ user, onReset }) {
  ...
  return <section id="owner-controls"><div className="section-label"><div><p className="kicker gold">ACCESS MANAGEMENT</p><h2>People & permissions.</h2></div><button type="button" className="quiet-button" disabled={creating} onClick={onReset}>Reset people view</button></div><p>Find an existing customer by name or email, or use assisted onboarding to create a client account for someone who needs help. Member access is separate from Staff and Administrator work permissions.</p><Notice error>{error}</Notice>
