...
export default function StaffBooking({ team, onSaved }) {
  ...
  return <details className="panel staff-create"><summary>Add a visit for a client</summary>
    ...
    {client && <form key={client._id} onSubmit={save}><div className="form-grid">
      <label>Service<select name="service">{catalog.filter(service => service.id !== 'online' && service.enabled !== false).map(service => <option key={service.id} value={service.id}>{service.name} · {money(service.cents)}{service.interval === 'walk' ? '/dog · 30 min' : service.interval === 'month' ? '/month' : ''}</option>)}</select></label>
