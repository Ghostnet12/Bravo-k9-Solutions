...
export default function StaffBooking({ team, onSaved }) {
  ...
  return <details className="panel staff-create"><summary>Add a visit for a client</summary>
    <p>Find their existing account, choose a trainer, and select one of that trainer’s available times. Saved visits appear on the client’s and team’s calendars.</p>
    ...
    {client && <form key={client._id} onSubmit={save}><div className="form-grid">
      ...
      <label>Trainer<select name="staffId" value={assignedTrainer} onChange={e => setAssignedTrainer(e.target.value)}><option value="">Unassigned</option>{trainerOptions(team).map(person => <option key={person._id} value={person._id} disabled={person.disabled}>{person.name}</option>)}</select></label>
