...
export default function StaffBooking({ team, onSaved }) {
  ...
  async function save(e) {
    ...
    try {
      await api('/admin/bookings', { method: 'POST', body: { requestKey, userId: client._id, staffId: fields.staffId || null, serviceIds: [fields.service], visits: [{ date, time, service: fields.service }], dogName: fields.dogName, phone: fields.phone, address: fields.address, notes: fields.notes } });
  ...
  return <details className="panel staff-create"><summary>Add a visit for a client</summary>
    ...
    {client && <form key={client._id} onSubmit={save}><div className="form-grid">
      <label>Program<select name="service"><option value="training">Professional training</option><option value="aggression">Aggressive-dog intake</option></select></label>
    ...
    </div><label>Visit address<input name="address" required minLength="5" maxLength="300" defaultValue={client.address}/></label><label>Private booking notes<textarea name="notes" maxLength="1500" rows="3"/></label><button className="button" disabled={busy || !time}>{busy ? 'Working…' : 'Add visit request'}</button></form>}
  </details>;
}
