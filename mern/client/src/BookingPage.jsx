...
export default function BookingPage() {
  ...
  const pricing = quote(ids, visits, { dogCount }, catalog);
  ...
  useEffect(() => { if (!editId && !saved) setBookingDraft({ ids, visits, kind, from, to, startTime, endTime, count, preference, dogCount, form, requestKey }); }, [editId, saved, ids, visits, kind, from, to, startTime, endTime, count, preference, dogCount, form, requestKey, setBookingDraft]);
  ...
  useEffect(() => {
    ...
    source.then(({ bookings }) => {
      ...
      setIds(booking.serviceIds); setVisits(booking.visits); setKind(booking.visits[0]?.service || 'training'); setDogCount(booking.dogCount || 1);
  ...
  async function save(e) {
    ...
    try {
      await api('/quote', { method: 'POST', body: { serviceIds: ids, visits, dogCount } });
      ...
      else { const { booking } = await api('/bookings', { method: 'POST', body: { requestKey, serviceIds: ids, visits, dogCount, ...form } }); setSaved(booking); setBookingDraft(null); setNotice('Request saved. Bravo will confirm the visit details. No card has been charged.'); }
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  }
  async function pay() { if (!saved || !config?.paymentsReady) return; setBusy(true); setError(''); try { const result = await api(`/bookings/${saved._id}/checkout`, { method: 'POST', body: {} }); window.location.assign(result.url); } catch (e) { setError(e.message); setBusy(false); } }
  return <Page title={editId ? 'Adjust your visits.' : 'Let’s plan your visit.'} className="booking-page" eyebrow="BOOK WITH BRAVO" intro="Choose your program, review available visits, and send your request to Bravo. All times are local to Aberdeen, South Dakota.">
    <nav className="booking-steps" aria-label="Booking sections"><a href="#choose-program"><span>01</span> Your program</a>{!!kinds.length && <a href="#choose-dates"><span>02</span> Your dates</a>}<a href="#visit-details"><span>{kinds.length ? '03' : '02'}</span> Your details</a><a className="booking-review-link" href="#your-plan">Review plan <span aria-hidden="true">↘</span></a></nav><SetupNotice/>{config?.paymentsMode === 'test' && <Notice>This is a Stripe test checkout. It cannot accept a real payment.</Notice>}<AppointmentNotice/><Notice error>{error}</Notice><Notice>{notice}</Notice><div className="booking-layout"><div className="booking-main">
      <section className="panel" id="choose-program"><div className="step-heading"><span>01</span><h2>Choose a service</h2></div><p>Pick the path that fits your dog. Pricing stays visible from the first tap.</p><div className="program-choices">{catalog.filter(service => service.enabled !== false).map(s => <button key={s.id} type="button" className={`program-choice ${ids.includes(s.id) ? 'is-selected' : ''}`} disabled={!!editId} aria-pressed={ids.includes(s.id)} onClick={() => select(s.id)}><ServiceIcon service={s.id}/><span><strong>{s.name}</strong>{ids.includes(s.id) && <span className="selection-mark" aria-hidden="true">✓</span>}<small>{s.description}</small></span><b>{money(s.cents)}<small>{s.interval === 'month' ? '/month' : s.interval === 'walk' ? '/dog · 30 min' : ' initial intake'}</small></b></button>)}</div>{ids.includes('walking') && <div className="dog-count"><span><strong>Number of dogs</strong><small>{money(catalog.find(service => service.id === 'walking')?.cents || 2500)} per dog for each 30-minute walk.</small></span><div className="dog-count-stepper"><button type="button" aria-label="Remove one dog" disabled={dogCount <= 1} onClick={() => setDogCount(value => Math.max(1, value - 1))}>−</button><input aria-label="Number of dogs for each walk" type="number" min="1" max="10" value={dogCount} onChange={e => setDogCount(Math.max(1, Math.min(10, Number(e.target.value) || 1)))}/><button type="button" aria-label="Add one dog" disabled={dogCount >= 10} onClick={() => setDogCount(value => Math.min(10, value + 1))}>+</button></div></div>}</section>
      ...
      <div className="form-grid"><label>Start date<input type="date" min={today()} max={addDays(today(), 92)} value={from} onChange={e => { setFrom(e.target.value); setUncovered([]); }}/></label><label>End date<input type="date" min={from} max={addDays(today(), 92)} value={to} onChange={e => { setTo(e.target.value); setUncovered([]); }}/></label><label>First visit no earlier than<select value={startTime} onChange={e => setStartTime(e.target.value)}>{hours.map(t => <option key={t}>{t}</option>)}</select></label><label>Last visit no later than<select value={endTime} onChange={e => setEndTime(e.target.value)}>{hours.map(t => <option key={t}>{t}</option>)}</select></label><label>Number of visits<input type="number" min="1" max="31" value={count} onChange={e => setCount(Number(e.target.value))}/></label><label>Preferred time<select value={preference} onChange={e => setPreference(e.target.value)}><option value="any">Any available time</option><option value="morning">Morning • 9–11 AM</option><option value="afternoon">Afternoon • noon–4 PM</option><option value="evening">Evening • 5–9 PM</option></select></label></div>
      <button className="button" type="button" disabled={busy || !config?.connected || !config?.schedule?.enabled} onClick={auto}>Find available days & times</button><p className="helper">Time preferences are flexible if no matching opening exists.</p>
      ...
      <section className="panel" id="visit-details"><div className="step-heading"><span>{kinds.length ? '03' : '02'}</span><h2>{editId ? 'Review your update' : 'Your visit details'}</h2></div>{!user ? <Notice><Link className="inline-link" to={`/account?next=${encodeURIComponent('/portal?resume=1')}`}>Sign in or create your account</Link> before saving a request. Your plan will still be here when you return. No payment details are collected on this page.</Notice> : <form onSubmit={save}><div className="form-grid">{['dogName', 'phone', 'address'].map(field => <label className={field === 'address' ? 'full-width' : ''} key={field}>{{ dogName: ids.includes('walking') && dogCount > 1 ? 'Dogs’ names' : 'Dog’s name', phone: 'Contact phone', address: 'Visit address' }[field]}<input value={form[field]} disabled={!!editId} required={field !== 'address' || kinds.length > 0} type={field === 'phone' ? 'tel' : 'text'} maxLength={field === 'address' ? 300 : field === 'phone' ? 30 : 80} onChange={e => setForm(f => ({ ...f, [field]: e.target.value }))}/></label>)}</div><label>Anything the team should know?<textarea maxLength="1500" rows="3" disabled={!!editId} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}/></label><p className="helper">Tell us about training goals, access instructions, and behavior needs. Don’t include medical records or sensitive information in the Bravo Room.</p><AppointmentNotice compact/><button className="button" disabled={busy || !config?.connected || !!saved}>{busy ? 'Working…' : saved ? 'Request saved' : editId ? 'Save schedule changes' : 'Save request • no charge'}</button></form>}</section>
    </div><aside className="order-summary panel" id="your-plan"><p className="kicker gold">YOUR BRAVO PLAN</p><h2>Your plan, at a glance.</h2>{pricing.lines.map(l => <div className="price-row" key={l.id}><span>{l.name}{l.id === 'walking' && <small>{dogCount} {dogCount === 1 ? 'dog' : 'dogs'} × {visits.filter(v => v.service === 'walking').length} {visits.filter(v => v.service === 'walking').length === 1 ? 'walk' : 'walks'} × {money(l.unitCents)}<br/>30 minutes per walk</small>}</span><strong>{money(l.unitCents * l.quantity)}</strong></div>)}{pricing.monthlyCents > 0 && <div className="price-total"><span>Monthly program</span><strong>{money(pricing.monthlyCents)}</strong></div>}{pricing.oneTimeCents > 0 && <div className="price-row"><span>Scheduled services total</span><strong>{money(pricing.oneTimeCents)}</strong></div>}<p>{visits.length} selected visits · {config?.paymentsReady ? 'Secure checkout opens after saving.' : 'Bravo will confirm payment separately.'}</p><p className="helper">Monthly programs renew until cancelled. Visits are subject to availability and Bravo confirmation.</p><AppointmentNotice compact/>{saved && <section className="checkout-ready" aria-labelledby="checkout-ready-title"><div className="checkout-secure-mark" aria-hidden="true">✓</div><p className="kicker gold">REQUEST SAVED</p><h3 id="checkout-ready-title">Ready for secure checkout.</h3><div className="checkout-due"><span>Due today</span><strong>{money(saved.quote?.dueNowCents ?? pricing.dueNowCents)}</strong></div>{pricing.monthlyCents > 0 && <p className="checkout-term"><strong>{money(pricing.monthlyCents)}/month</strong> renews automatically until cancelled.</p>}{pricing.oneTimeCents > 0 && <p className="checkout-term"><strong>{money(pricing.oneTimeCents)}</strong> in scheduled service charges is included today.</p>}<p className="helper">Payment is completed on Stripe’s secure checkout. Bravo never receives or stores your full card number.</p>{config?.paymentsReady ? <button className="button checkout-button" type="button" disabled={busy} onClick={pay}>{busy ? 'Opening secure checkout…' : <>Continue to secure checkout <span aria-hidden="true">→</span></>}</button> : <Notice>Online payment is temporarily unavailable. Your request is saved and Bravo will contact you.</Notice>}<Link className="inline-link" to="/account">View my saved request →</Link></section>}{!config?.paymentsReady && !saved && <p className="helper">Bravo will follow up about payment after confirming your request.</p>}</aside></div><div className="mobile-booking-bar"><span>{saved ? <><strong>{money(saved.quote?.dueNowCents ?? pricing.dueNowCents)} due today</strong><small>Request saved · secure payment next</small></> : ids.includes('walking') ? <><strong>{dogCount} {dogCount === 1 ? 'dog' : 'dogs'} × {money(catalog.find(service => service.id === 'walking')?.cents || 2500)}</strong><small>30-minute walk · {visits.length ? `${visits.length} selected` : 'choose a time'}</small></> : <><strong>{pricing.lines.map(line => line.name).join(' + ')}</strong><small>{visits.length ? `${visits.length} selected visits` : 'Choose your schedule'}</small></>}</span>{saved && config?.paymentsReady ? <button className="button" type="button" disabled={busy} onClick={pay}>{busy ? 'Opening…' : 'Secure checkout'} <span aria-hidden="true">→</span></button> : <a className="button" href={visits.length || !kinds.length ? '#visit-details' : '#choose-dates'}>{visits.length || !kinds.length ? 'Review details' : 'Choose a time'} <span aria-hidden="true">→</span></a>}</div>
  </Page>;
}
