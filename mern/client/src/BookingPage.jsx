...
export default function BookingPage() {
  ...
  useEffect(() => { if (!editId && !saved) setBookingDraft({ ids, visits, kind, trainingFocus, from, to, startTime, endTime, count, preference, dogCount, form, requestKey }); }, [editId, saved, ids, visits, kind, trainingFocus, from, to, startTime, endTime, count, preference, dogCount, form, requestKey, setBookingDraft]);
  ...
  useEffect(() => {
    ...
    source.then(({ bookings }) => {
      ...
      setEditingBooking(booking); setIds(booking.serviceIds); setVisits(booking.visits); setKind(booking.visits[0]?.service || 'training'); setTrainingFocus(booking.trainingFocus || 'basic-obedience'); setDogCount(booking.dogCount || 1);
  ...
  async function save(e) {
    ...
    try {
      ...
      else { const { booking } = await api('/bookings', { method: 'POST', body: { requestKey, serviceIds: ids, visits, trainingFocus: ids.includes('training') ? trainingFocus : undefined, dogCount, ...form } }); setSaved(booking); setBookingDraft(null); setNotice('Request saved. Bravo will confirm the visit details. No card has been charged.'); }
