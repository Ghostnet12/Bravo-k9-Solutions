...
export default function BookingPage() {
  ...
  return <Page title={editId ? 'Adjust your visits.' : 'Let’s plan your visit.'} className="booking-page" eyebrow="BOOK WITH BRAVO" intro="Choose your program, review available visits, and send your request to Bravo. All times are local to Aberdeen, South Dakota.">
    <nav className="booking-steps" aria-label="Booking sections"><a href="#choose-program"><span>01</span> Your program</a>{!!kinds.length && <a href="#choose-dates"><span>02</span> Your dates</a>}<a href="#visit-details"><span>{kinds.length ? '03' : '02'}</span> Your details</a><a className="booking-review-link" href="#your-plan">Review plan <span aria-hidden="true">↘</span></a></nav><SetupNotice/><AppointmentNotice/><Notice error>{error}</Notice><Notice>{notice}</Notice><div className="booking-layout"><div className="booking-main">
      <section className="panel" id="choose-program"><div className="step-heading"><span>01</span><h2>Choose your program</h2></div><p>Select the training or care path that fits your goals.</p><div className="program-choices">{catalog.filter(service => service.enabled !== false).map(s => <button key={s.id} type="button" className={`program-choice ${ids.includes(s.id) ? 'is-selected' : ''}`} disabled={!!editId} aria-pressed={ids.includes(s.id)} onClick={() => select(s.id)}><span><strong>{s.name}</strong>{ids.includes(s.id) && <span className="selection-mark" aria-hidden="true">✓</span>}<small>{s.description}</small></span><b>{money(s.cents)}<small>{s.interval === 'month' ? '/month' : s.interval === 'walk' ? '/dog · 30 min' : s.interval === 'day' ? '/day' : ' initial intake'}</small></b></button>)}</div>{ids.includes('walking') && <label className="dog-count">Number of dogs for each walk<input type="number" min="1" max="10" value={dogCount} onChange={e => setDogCount(Math.max(1, Math.min(10, Number(e.target.value) || 1)))}/><small>{money(catalog.find(service => service.id === 'walking')?.cents || 2500)} per dog for each 30-minute walk.</small></label>}</section>
