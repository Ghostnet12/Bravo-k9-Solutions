...
export default function BookingPage() {
  ...
  const [ids, setIds] = useState([SERVICES.some(s => s.id === params.get('program')) ? params.get('program') : 'training']);
  const [visits, setVisits] = useState([]), [kind, setKind] = useState(params.get('program') === 'aggression' ? 'aggression' : 'training');
  ...
  const [count, setCount] = useState(4), [preference, setPreference] = useState('any');
  ...
  return <Page title={editId ? 'Adjust your visits.' : 'Let’s plan your visit.'} className="booking-page" eyebrow="BOOK WITH BRAVO" intro="Choose your program, review available visits, and send your request to Bravo. All times are local to Aberdeen, South Dakota.">
    <nav className="booking-steps" aria-label="Booking sections"><a href="#choose-program"><span>01</span> Your program</a>{!!kinds.length && <a href="#choose-dates"><span>02</span> Your dates</a>}<a href="#visit-details"><span>{kinds.length ? '03' : '02'}</span> Your details</a><a className="booking-review-link" href="#your-plan">Review plan <span aria-hidden="true">↘</span></a></nav><SetupNotice/><Notice error>{error}</Notice><Notice>{notice}</Notice><div className="booking-layout"><div className="booking-main">
      <section className="panel" id="choose-program"><div className="step-heading"><span>01</span><h2>Choose your program</h2></div><p>Select the training path that fits your goals.</p><div className="program-choices">{SERVICES.map(s => <button key={s.id} type="button" className={`program-choice ${ids.includes(s.id) ? 'is-selected' : ''}`} disabled={!!editId} aria-pressed={ids.includes(s.id)} onClick={() => select(s.id)}><span><strong>{s.name}</strong>{ids.includes(s.id) && <span className="selection-mark" aria-hidden="true">✓</span>}<small>{s.description}</small></span><b>{money(s.cents)}<small>{s.interval === 'month' ? '/month' : ' initial intake'}</small></b></button>)}</div></section>
