...
export default function BookingPage({ sitting = false }) {
  ...
  const [ids, setIds] = useState([sitting ? 'sitting' : params.get('program') === 'online' ? 'online' : 'training']);
  const [visits, setVisits] = useState([]), [kind, setKind] = useState(sitting ? 'sitting' : 'training');
  ...
  return <Page title={sitting ? 'Good care. While you’re away.' : editId ? 'Adjust your visits.' : 'Your dog. Your program.'} eyebrow="CLIENT SCHEDULING" intro="Choose your program, review available visits, and send your request to Bravo. All times are local to Aberdeen, South Dakota.">
    <SetupNotice/><Notice error>{error}</Notice><Notice>{notice}</Notice><div className="booking-layout"><div className="booking-main">
      <section className="panel"><div className="step-heading"><span>01</span><h2>Choose your programs</h2></div><p>Combine individual services or select one complete package.</p><div className="program-choices">{SERVICES.map(s => <button key={s.id} type="button" className={`program-choice ${ids.includes(s.id) ? 'is-selected' : ''}`} disabled={!!editId} aria-pressed={ids.includes(s.id)} onClick={() => select(s.id)}><span><strong>{s.name}</strong><small>{s.description}</small></span><b>{money(s.cents)}<small>{s.interval === 'month' ? '/month' : s.interval === 'day' ? '/day' : 'initial intake'}</small></b>{s.bundle && <em>PACKAGE</em>}</button>)}</div>{!sitting && <Link className="inline-link" to="/portal/dog-sitting">Only need dog sitting? Open the care scheduler →</Link>}</section>
      {!!kinds.length && <section className="panel"><div className="step-heading"><span>02</span><h2>Build your schedule</h2></div><p>Each visit has its own date and time. Requests use one available hourly slot per visit.</p>{kinds.length > 1 && <div className="tab-row" aria-label="Visit type">{kinds.map(k => <button key={k} aria-pressed={kind === k} onClick={() => setKind(k)} className={kind === k ? 'selected' : ''}>{k === 'sitting' ? 'Dog sitting' : 'Training'} visits</button>)}</div>}
      ...
      {loadingDays ? <p role="status">Checking availability…</p> : <div className="date-grid">{days.map(day => { const selected = visits.some(v => v.date === day.date && v.service === kind); return <button key={day.date} disabled={!day.slots.length && !selected} aria-pressed={selected} className={selected ? 'selected' : ''} onClick={() => addVisit(day)}><strong>{formatDate(day.date)}</strong><small>{selected ? 'Selected' : day.slots.length ? `${day.slots.length} openings` : 'No openings'}</small></button>; })}</div>}
