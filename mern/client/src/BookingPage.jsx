...
export default function BookingPage() {
  ...
  const [trainerId, setTrainerId] = useState(resume?.trainerId || ((params.get('trainer') === SHARED_TRAINER_ID || /^[a-f\d]{24}$/i.test(params.get('trainer') || '')) ? params.get('trainer') : '')), [trainers, setTrainers] = useState([]), [trainersLoading, setTrainersLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    source.then(({ bookings }) => {
      ...
      setEditingBooking(booking); setIds(booking.serviceIds); setVisits(booking.visits); setKind(booking.visits[0]?.service || 'training'); setTrainingFocus(booking.trainingFocus || 'basic-obedience'); setTrainerId(selectedTrainerId(booking)); setDogCount(booking.dogCount || 1);
