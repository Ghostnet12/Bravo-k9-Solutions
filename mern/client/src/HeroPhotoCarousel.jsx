...
export default function HeroPhotoCarousel({ children }) {
  ...
  useEffect(() => {
    if (paused || reduced || open || editingVideo || !inView || keys.length < 2) return;
    const timer = setInterval(() => {
      if (!document.hidden && !held.current && !editingPhoto.current && !(keyboardFocus.current && root.current?.contains(document.activeElement)) && !document.querySelector('[data-site-image-editor][open]') && ![...(root.current?.querySelectorAll('video') || [])].some(video => !video.paused && !video.ended)) setIndex(value => (value + 1) % keys.length);
    }, settings.intervalSeconds * 1000);
    return () => clearInterval(timer);
  }, [paused, reduced, open, editingVideo, inView, keys.length, settings.intervalSeconds]);
  ...
  return <div ref={root} className="home-hero-gallery" role="region" aria-label="Trainer photos" tabIndex={canEdit ? 0 : undefined}
    ...
    onPointerDownCapture={event => { keyboardFocus.current = false; if (event.target.closest('dialog,button,input')) return; held.current = true; if (!canEdit || event.button !== 0) return; event.stopPropagation(); cancelHold(); held.current = true; hold.current = { x: event.clientX, y: event.clientY, timer: setTimeout(showEditor, 650) }; }}
    ...
    <div className="hero-photo-window"><div className="hero-photo-track" style={{ transform: `translateX(-${index * 100}%)`, transition: reduced || index === 0 ? 'none' : undefined }}>
      ...
      {settings.photos.map((key, i) => { const item = photo(key); return <div key={key} className="hero-photo-slide" aria-hidden={index !== i + 1} inert={index !== i + 1}>{heroVideoId(key) ? <HeroVideo clip={videos[heroVideoId(key)]} active={index === i + 1 && inView && !open && !editingVideo} paused={paused || reduced} onEnded={() => setIndex(value => (value + 1) % keys.length)}/> : <img className="hero-carousel-image" src={item.src} alt={item.alt} width="828" height="1121" loading="lazy" data-site-image-key={key} data-site-image-original={HERO_PHOTO_DEFAULTS[key]?.src || item.src} style={images[key]?.framed ? framingStyle(images[key]) : undefined}/>}</div>; })}
    </div></div>
    {keys.length > 1 && <div className="hero-photo-controls"><button type="button" aria-label="Previous trainer photo" onClick={() => setIndex(value => (value - 1 + keys.length) % keys.length)}>←</button><button type="button" aria-label={paused ? 'Resume trainer photos' : 'Pause trainer photos'} onClick={() => setPaused(value => !value)}>{paused ? '▶' : 'Ⅱ'}</button><button type="button" aria-label="Next trainer photo" onClick={() => setIndex(value => (value + 1) % keys.length)}>→</button></div>}
