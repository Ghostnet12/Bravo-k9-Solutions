...
export default function BulkTimes({ dates, options, available, onApply, disabled = false }) {
  ...
  return <fieldset className="bulk-times" disabled={disabled}>
    ...
    <div className="bulk-time-buttons">{times.map(time => {
      ...
      return <div key={time}><button type="button" className="button button-ghost" disabled={blocked.length > 0} onClick={() => onApply(time)}>{formatTime(time)}</button>{blocked.length > 0 && <small>Unavailable: {blocked.map(formatDate).join(', ')}</small>}</div>;
    })}</div>
