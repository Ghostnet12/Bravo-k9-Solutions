...
export function Header() {
  const { user } = useBravo();
  return <header className="app-header"><Brand/><nav aria-label="Primary navigation"><NavLink to="/">Home</NavLink><NavLink to="/portal">Book a visit</NavLink><NavLink to="/learn">Online training</NavLink><NavLink to="/community">Bravo Room</NavLink><NavLink to="/account">{user ? 'My account' : 'Sign in'}</NavLink>{user?.role === 'staff' && <NavLink to="/admin">Staff desk</NavLink>}</nav></header>;
}
export function Footer() { return <footer className="app-footer shell"><span>© {new Date().getFullYear()} Bravo K9 Solutions, LLC</span><NavLink to="/media-rights">Media permissions</NavLink><a href="tel:+16058242767">(605) 824-2767</a><span>TRUST. TRAIN. DEPLOY.</span></footer>; }
