...
function ContentEditor({ options, entries, publish, close, preview }) {
  ...
  return <dialog ref={dialog} className="site-content-dialog" data-site-image-ignore="" aria-label="Edit website section" onCancel={event=>{event.preventDefault();if(!busy)close();}}>
    <form onSubmit={event=>{event.preventDefault();save();}}><div className="site-content-heading"><h2>Edit website</h2><button type="button" disabled={busy} onClick={close} aria-label="Close website editor">×</button></div>
      ...
      <fieldset disabled={busy}><label>Edit this part<select value={key} onChange={event=>{setKey(event.target.value);setDraft(entries[event.target.value]?.value || {});setError('');}}>{options.map(item=><option key={item.key} value={item.key}>{item.title}</option>)}</select></label>
      {selected.text && <label>Text<textarea rows={5} maxLength={8000} value={draft.text ?? selected.original} onChange={event=>change('text',event.target.value)}/></label>}
      ...
      <div className="site-content-fields"><label>Font<select value={draft.font || ''} onChange={event=>setDraft(({font,...old})=>event.target.value?{...old,font:event.target.value}:old)}><option value="">Original font</option>{Object.keys(CONTENT_FONTS).map(font=><option key={font} value={font}>{font}</option>)}</select></label>
      ...
      <label>Text alignment<select value={draft.textAlign || ''} onChange={event=>setDraft(({textAlign,...old})=>event.target.value?{...old,textAlign:event.target.value}:old)}><option value="">Original</option><option>left</option><option>center</option><option>right</option></select></label>
      <label>Background<select value={draft.background || ''} onChange={event=>setDraft(({background,...old})=>event.target.value?{...old,background:event.target.value}:old)}><option value="">Original background</option><option value="solid">Solid color</option><option value="gradient">Gradient</option><option value="transparent">Transparent</option></select></label>
  ...
  useEffect(()=>{
    ...
    async function open(target){
      ...
      for(let node=target.closest('[data-site-content-key]');node;node=node.parentElement?.closest('[data-site-content-key]')){
        ...
        options.push({key:node.dataset.siteContentKey,text:node.dataset.siteContentText==='true',link:node.dataset.siteOriginalLink,original:node.dataset.siteOriginalText || '',title:`${node.tagName.toLowerCase()}: ${(node.dataset.siteOriginalText || node.getAttribute('aria-label') || node.className || 'section').replace(/\s+/g,' ').slice(0,65)}`});
      }
      for (const node of target.querySelectorAll('[data-site-content-key]')) { if(options.some(item=>item.key===node.dataset.siteContentKey))continue; options.push({key:node.dataset.siteContentKey,text:node.dataset.siteContentText==='true',link:node.dataset.siteOriginalLink,original:node.dataset.siteOriginalText || '',title:`${node.tagName.toLowerCase()}: ${(node.dataset.siteOriginalText || node.className || 'section').replace(/\s+/g,' ').slice(0,65)}`}); }
