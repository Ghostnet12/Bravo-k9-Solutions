...
export default function MembershipDateFields({ optional = false, initialStart, initialServices = ['online'], initialDogCount = 1, disabled = false }) {
  ...
  return <fieldset className="membership-date-fields" disabled={disabled}>
    ...
    {enabled && <><div className="form-grid">
      <label>Membership start date<input name="membershipStartDate" type="date" value={start} onChange={e => setStart(e.target.value)} required/><small>Past, present, or future dates are allowed.</small></label>
      <label>Membership end date<input type="date" value={dates?.endDate || ''} readOnly/><small>Automatically ends at the start of this date, Aberdeen time.</small></label>
      <label>Member services<select name="membershipServices" value={services} onChange={e => setServices(e.target.value)}><option value="training">Training membership</option><option value="online">Online lessons</option><option value="training,online">Training and online lessons</option></select></label>
      {services.includes('training') && <label>Dogs covered<input type="number" name="membershipDogCount" min="1" max="10" defaultValue={initialDogCount} required/></label>}
