...
export default function MembershipDateFields({ value, onChange, disabled = false, optional = false, name, label = 'Membership start date' }) {
  const id = useId();
  const start = value || (optional ? membershipToday() : '');
  let end = '';
  try { if (start) end = membershipDate(manualMonthTerm(start).validUntil); } catch { /* The input and server validate the chosen date. */ }
