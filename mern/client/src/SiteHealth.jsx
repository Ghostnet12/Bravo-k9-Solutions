...
export default function SiteHealth() {
  ...
  return <section aria-labelledby="site-health-title" className="site-health">
    ...
    {data && <>
      ...
      {!!data.channels.length && <section className="panel"><h3>Where visits come from</h3><div className="site-health-table"><table><thead><tr><th>Source</th><th>Visits</th><th>Requests</th><th>Saved rate</th></tr></thead><tbody>{data.channels.map(row => <tr key={row.channel}><th scope="row">{row.channel}</th><td>{row.visits}</td><td>{row.saved}</td><td>{(100 * row.saved / row.visits).toFixed(1)}%</td></tr>)}</tbody></table></div></section>}
