...
import { proofVideoType } from '../../shared/proof-videos.js';
...
export default function ProofVideoEditor({ clip, onClose, onSaved, onRemoved }: { clip: ProofClip; onClose: () => void; onSaved: (clip: ProofClip) => void; onRemoved: (id: string) => void }) {
  ...
  const changed = !!file || title !== clip.title || description !== clip.description || fit !== (clip.fit || 'contain');
  useEffect(() => {
  ...
  async function publish(event: React.FormEvent) {
    ...
    if (saving.current || !changed || !title.trim() || file && !ready || isNew && !file) return;
    saving.current = true; setBusy(true); setError('');
    const posterData = file ? thumbnail(preview.current) : undefined;
    ...
    try {
      ...
      const result = await api(`/proof-videos/${clip.id}`, { method: 'PUT', body: { expectedRevision: clip.revision, mutationId: mutation.current, title, description, fit, ...(file ? { uploadId: upload.current!.id, ...(posterData ? { posterData } : {}) } : {}) } });
  ...
  return <dialog ref={dialog} className="site-photo-dialog proof-video-dialog" data-site-image-editor="" aria-labelledby="proof-editor-title" aria-busy={busy} onCancel={event => { event.preventDefault(); if (!saving.current) onClose(); }}>
    <form onSubmit={publish}>
      ...
      <p className="site-photo-intro">Choose a video from your device, add its story, then publish it to the homepage.</p>
