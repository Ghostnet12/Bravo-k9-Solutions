...
export default function ClientTrainer({bookings,onSaved}) {
  const {user}=useBravo();
  const [bookingId,setBookingId]=useState(bookings[0]?._id||''),[trainer,setTrainer]=useState(bookings[0]?.staffId||bookings[0]?.requestedStaffId||user.id),[team,setTeam]=useState(null),[error,setError]=useState(''),[busy,setBusy]=useState(false);
  const booking=bookings.find(b=>b._id===bookingId),assigned=booking?.staffId||booking?.requestedStaffId;
  async function load(){try{const r=await api('/team');setTeam(r.team)}catch(e){setError(e.message)}}
  async function save(accept) {
    setBusy(true);setError('');try{
      if(accept){const r=await api(`/admin/bookings/${bookingId}/accept-client`,{method:'POST',body:{staffId:trainer,revision:booking.updatedAt}});onSaved(r.message)}
