...
const mediaRules = { video: { accept: 'video/mp4,video/webm', label: 'Lesson video', max: 80 * 1024 * 1024 }, captions: { accept: '.vtt,text/vtt', label: 'English captions (.vtt)', max: 2 * 1024 * 1024 }, image: { accept: 'image/jpeg,image/png,image/webp', label: 'Cover photo', max: 8 * 1024 * 1024 } };
...
export default function LessonEditor() {
  ...
  async function upload(kind, file) {
    if (!file) return; const rule = mediaRules[kind];
  ...
  return <section id="lesson-editor" className="panel lesson-studio">
    ...
    <p>Create the lesson first, then upload its cover, video, and captions. Nothing is visible to members until you publish.</p><Notice error>{error}</Notice><Notice>{notice || progress}</Notice>
    ...
    {draft && <form onSubmit={e => { e.preventDefault(); save(false); }}><div className="form-grid"><label>Lesson ID<input value={draft._id} disabled={!isNew || busy} onChange={e => field('_id', e.target.value)} pattern="[a-z0-9-]{1,80}" maxLength="80" required placeholder="loose-leash-basics"/></label><label>Title<input value={draft.title} onChange={e => field('title', e.target.value)} maxLength="120" required/></label><label>Topic<input value={draft.category} onChange={e => field('category', e.target.value)} maxLength="40" required/></label><label>Instructor<input value={draft.instructor} onChange={e => field('instructor', e.target.value)} maxLength="80" required/></label><label>Built-in cover<select value={draft.image.startsWith('/images/') ? draft.image : '/images/training-education.webp'} onChange={e => field('image', e.target.value)}>{images.map(i => <option key={i} value={`/images/${i}.webp`}>{i.replaceAll('-', ' ')}</option>)}</select></label></div>
      ...
      {!isNew && <section className="media-uploader" aria-label="Lesson media uploads">{Object.entries(mediaRules).map(([kind, rule]) => <div className="media-upload-row" key={kind}><div><strong>{rule.label}</strong><small>{draft.media?.[kind]?.filename || 'Not uploaded yet'}</small></div><label className="button button-small button-ghost">{draft.media?.[kind] ? 'Replace' : 'Upload'}<input className="sr-only" type="file" accept={rule.accept} disabled={busy} onChange={e => { upload(kind, e.target.files?.[0]); e.target.value = ''; }}/></label>{draft.media?.[kind] && <button type="button" className="quiet-button" disabled={busy} onClick={() => removeMedia(kind)}>Remove</button>}</div>)}</section>}
      {isNew && <Notice>Save this lesson as a draft to unlock media uploads.</Notice>}
